        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8"> {{-- Menyesuaikan lebar dan padding agar konsisten --}}
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg"> {{-- Menambahkan dukungan dark mode --}}
                <div class="p-6">
                    ...
                    <form action="{{ route('admin.doctor.update', $doctorPage->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="content" id="content" rows="5"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100">{{ old('content', $doctorPage->content) }}</textarea>
                            @error('content')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
